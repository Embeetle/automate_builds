...
def _help() -> None:
    ...
    print(f"This tool automates building the Embeetle components (LLVM, SA and Embeetle itself),")
    print(f"all the way from cloning the repos to the final builds. Runs on Windows in a standard")
    print(f"Windows CMD shell (not in Powershell, MSYS shell or whatever).")
    print(f"")
    ...
    print(f"          As said before, this script should run in a standard Windows CMD")
    print(f"          shell, but it will launch MSYS2 sub-shells when needed.")
    print(f"    ")
    ...
    print(f"    ")
    print(f"        Clones all repos, installs MSYS2 packages, and builds LLVM, SA, and")
    print(f"        Embeetle in one go. Find the resulting executable at:")
    print(f"    ")
    print(f"            {c('~/bld/embeetle-windows-x86_64/embeetle.exe', fg='bright_yellow')}")
    print(f"    ")
    ...
    print(f"                       environment at {c('<EMBEETLE_REPO>/.venv', fg='bright_yellow')} and safely")
    print(f"                       installs all required dependencies in isolation.")
    print(f"    ")
    print(f"    {c('--all', fg='bright_cyan')}              Run the full build pipeline:")
    ...
    print(f"                           Requires GitHub write access (checked via token).")
    print(f"    ")
    print(f"    {c('--upload', fg='bright_cyan')}            Upload the {c('<BUILD_DIR>/embeetle-<PLATFORM>.7z', fg='bright_yellow')} archive")
    print(f"                           to a GitHub Release for the version in version.txt.")
    ...
    print(f"              │   └─ sa")
    print(f"              ├─ embeetle  ")
    print(f"              │   └─ .venv  {c('<-- Isolated Python environment', fg='bright_black')}")
    print(f"              ├─ llvm  ")
    print(f"              └─ sa  ")
    print(f"    ")
    print(f"    To run Embeetle..")
    print(f"    ")
    print(f"        {c('..from sources (Windows CMD):', fg='bright_magenta')}")
    print(f"            Open a Windows CMD terminal and move into the repo at <MSYS2_HOME>/embeetle:")
    print(f"                {c('>', fg='bright_green')} {c('cd embeetle', fg='bright_yellow')}")
    print(f"    ")
    print(f"            If a {c('.venv', fg='bright_yellow')} subfolder doesn't exist yet, create and populate it:")
    print(f"                {c('>', fg='bright_green')} {c('python -m venv .venv', fg='bright_yellow')}")
    print(f"                {c('>', fg='bright_green')} {c('call .venv/Scripts/activate.bat', fg='bright_yellow')}")
    print(f"                {c('>', fg='bright_green')} {c('python -m pip install -r requirements.txt', fg='bright_yellow')}")
    print(f"    ")
    print(f"            Launch Embeetle:")
    print(f"                {c('>', fg='bright_green')} {c('run.cmd', fg='bright_yellow')}")
    print(f"    ")
    print(f"        {c('..from sources (Windows PowerShell):', fg='bright_magenta')}")
    print(f"            Open a Windows PowerShell terminal and move into the repo at <MSYS2_HOME>/embeetle:")
    print(f"                {c('>', fg='bright_green')} {c('cd embeetle', fg='bright_yellow')}")
    print(f"    ")
    print(f"            If a {c('.venv', fg='bright_yellow')} subfolder doesn't exist yet, create and populate it:")
    print(f"                {c('>', fg='bright_green')} {c('python -m venv .venv', fg='bright_yellow')}")
    print(f"                {c('>', fg='bright_green')} {c('.venv\\Scripts\\activate.ps1', fg='bright_yellow')}")
    print(f"                {c('>', fg='bright_green')} {c('python -m pip install -r requirements.txt', fg='bright_yellow')}")
    print(f"    ")
    print(f"            Note: If you get a script execution error on the activation step, run this first:")
    print(f"                {c('>', fg='bright_green')} {c('Set-ExecutionPolicy RemoteSigned -Scope CurrentUser', fg='bright_yellow')}")
    print(f"    ")
    print(f"            Launch Embeetle:")
    print(f"                {c('>', fg='bright_green')} {c('run.ps1', fg='bright_yellow')}")
    print(f"    ")
    print(f"        {c('..from the executable:', fg='bright_magenta')}")
    print(f"            Navigate to the embeetle build at {c('<MSYS2_HOME>/bld/embeetle-<PLATFORM>', fg='bright_yellow')} and")
...
def setup_git_auth_from_token() -> None:
    ...
    We write a small .cmd script that returns 'x-access-token' for the username
    ...
    if not token:
        return
    askpass_path = Path(os.environ.get("TEMP", "C:/Windows/Temp")) / "_embeetle_git_askpass.cmd"
    askpass_path.write_text(
        "@echo off\n"
        'echo %1 | findstr /i "username" >nul && echo x-access-token && exit /b\n'
        'echo %1 | findstr /i "password" >nul && echo ' + token + ' && exit /b\n'
    )
...
def build_embeetle() -> None:
    ...
    venv_dir: Path = EMBEETLE_REPO / ".venv"
...
def main() -> int:
    ...
    if args.inc_version:
        printc("")
        printc("INC VERSION", fg="bright_blue")
        printc("===========", fg="bright_blue")
        inc_version()
